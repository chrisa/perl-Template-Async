...
subtest 'run process' => sub {
    ...
    like $output, qr/MSFT: [0-9.]+/
         => 'got expected output stash';

    like $output, qr/GOOG: [0-9.]+/
         => 'got expected output from async block';
};
...
[% ASYNC val = $rest.get('select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22GOOG%22)%0A%09%09&env=http%3A%2F%2Fdatatables.org%2Falltables.env&format=json') -%]
[% val.query.results.quote.symbol %]: [% val.query.results.quote.Ask %]
[% END -%]
EOT
}
